...
function openAddDlg(){
 ...
 function chooiseAdv(id){
		...
		$.post("adminAdv_updateAdv.action",{id:id},
			function(result){
...
function zoneDelete(id){
	if(confirm("确定要删除这条数据吗?")){
		$.post("adminAdv_deleteAdv.action",{id:id},
				function(result){
...
function deleteSections(){
	...
	if(confirm("您确定要删除这"+selectedSpan.length+"条数据吗？")){
		$.post("adminAdv_deleteAdvs.action",{ids:ids},function(result){
			if(result.success){
				alert("数据已成功删除！");
				location.reload(true);
			}else{
...
<body>
	<div class="container-fluid">
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			...
			<form action="adminAdv_findByStatus.action" method="post" class="form-search" style="display: inline;">      
			   &nbsp;广告状态：
		...
		<div id="dlg" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-body">
				<form id="fm" action="adminAdv_addAdv.action"  method="post" enctype="multipart/form-data">
					<table>
						<input type="hidden" id="logo" name="id">
						<tr>
							<td>
								<label class="control-label" for="logo">上传广告：</label>
							</td>
							<td>
								<input type="file" id="logo" name="adv">
								
							</td>
						</tr>
						<tr>
							<td>
								<label class="control-label" for="teacherId">状态</label>
							</td>
							<td>
						...
						<tr>
							<td>
								<label class="control-label" for="courseList">选择直播间：</label>
							</td>
							<td>
								 <select id="courseList" name="courseId">
									<option value="" >请选择</option>
									<c:forEach var="b" items="${courseList}">
