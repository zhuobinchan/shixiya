...
function openAddDlg(){
 ...
 function chooiseAdv(id){
		...
		$.post("updateAdv_AdminPreferenceSelectAdvAction.action",{id:id},
			function(result){
...
function zoneDelete(id){
	if(confirm("确定要删除这条数据吗?")){
		$.post("deleteAdv_AdminPreferenceSelectAdvAction.action",{id:id},
				function(result){
...
function deleteSections(){
	...
	if(confirm("您确定要删除这"+selectedSpan.length+"条数据吗？")){
		$.post("deleteAdvs_AdminPreferenceSelectAdvAction.action",{ids:ids},function(result){
			if(result.success){
				alert("数据已成功删除！");
				location.reload(true);
			}else{
...
<body>
	<div class="container-fluid">
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			...
			<form action="findByStatus_AdminPreferenceSelectAdvAction.action" method="post" class="form-search" style="display: inline;">      
			   &nbsp;广告状态：
		...
		<div id="dlg" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-body">
				<form id="fm" action="addAdv_AdminPreferenceSelectAdvAction.action"  method="post" enctype="multipart/form-data">
					<table>
						...
						<tr>
							<td>
								<label class="control-label" for="broadcastList">堂外优选汇：</label>
							</td>
							<td>
								 <select id="broadcastList" name="preferenceSelectId">
									<option value="" >请选择</option>
									<c:forEach var="b" items="${preferenceSelect}">
										<option value="${b.id }" >${b.workName }</option>
									</c:forEach>
			 					 </select>
							</td>
						</tr>
