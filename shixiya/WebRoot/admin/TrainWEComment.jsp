...
<script type="text/javascript">
	...
	function commentDelete(commentId) {
		if (confirm("确定要删除这条数据吗?")) {
			$.post("delete_AdminTrainWEComment.action", {
				id : commentId
			}, function(result) {
	...
	function deleteComments() {
		...
		if (confirm("您确定要删除这" + selectedSpan.length + "条数据吗？")) {
			$.post("deleteList_AdminTrainWEComment.action", {
				ids : ids
			}, function(result) {
				if (result.success) {
					alert("数据已成功删除！");
...
<body>
	<div class="container-fluid">
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			...
			<form action="getByCondition_AdminTrainWEComment.action" method="post" class="form-search"
							...
							<tbody>
								<c:forEach items="${comments }" var="comment" varStatus="idx">
									<tr>
										...
										<td style="text-align: center;vertical-align: middle;">${comment.trainWEProductChild.title }</td>
