...
<script type="text/javascript">
	$(function() {
		...
		$("#courseSelect").change(function(e){
		   	  ...
		   	  var chapterId = $(this).val();
		   	   $("#videoSelect").empty(); 
		   	  $.post("bComment_findVideo",{chapterId:chapterId},function(data){
		   	 	 var jsonobj = eval("(" + data + ")");
		   	 	 
   				 $.each(jsonobj, function (index, value) {
       			 	$("#videoSelect").append("<option value='" + value.id + "'> " + value.name + " </option>");
    			 });
	...
	function commentDelete(commentId) {
		if(confirm("确定要删除吗?")){
		$.post("bComment_deleteComment.action",{id:commentId},
				function(result){
	...
	function deleteComments() {
		...
		if (confirm("您确定要删除这" + selectedSpan.length + "条数据吗？")) {
			$.post("bComment_deleteComments.action", {ids:ids}, function(result) {
				if (result.success) {
					alert("数据已成功删除！");
...
<body>
	<div class="container-fluid">
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			...
			<form action="bComment_findCommentByCondition.action" method="post" class="form-search"
				...
				评论内容： <input name="commentContent" 
					...
					/> 
				&nbsp;
				章节名称：<select id="courseSelect" name="cselect">
							<option value="">请选择..</option>
							<c:forEach items="${chapters}" var="c">
								<option value="${c.id }">${c.title }</option>
							</c:forEach>
						</select> 
				&nbsp;视频名称<select id="videoSelect" name="videoId">
							...
							<tbody>
								<c:forEach items="${comments }" var="comment" varStatus="idx">
									<tr>
										...
										<td style="text-align: center;vertical-align: middle;">${comment.BProductVideo.title }</td>
