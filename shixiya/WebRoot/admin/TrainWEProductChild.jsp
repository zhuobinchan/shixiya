...
<script type="text/javascript">
	...
	function openAddDlg() {
		...
		$("#fm").attr("action","addTrainWEProductChild_AdminTrainWEChildAction.action");
		resetValue();
	}
	function saveTrainWEProduct() {
		...
		if ($("#trainWESelect").val() == null || $("#trainWESelect").val() == '') {
			$("#error").html("请选择所属课程");
			return false;
		}			
	...
	function modifyTrainWE(id, name,publisher, startTime,
			endTime, introduction,listViewContent, status,trainWeProductId, broadcastMode,broadcastParticipation,logo,visitnum) {
		resetValue();

		var introduction_removeEnter = introduction.replace(/\[enter\]/g,"\n");
		var listViewContent_removeEnter = listViewContent.replace(/\[enter\]/g,"\n");
		$("#fm").attr("action","updateTrainWEProductChild_AdminTrainWEChildAction.action");
		...
		$("#state").val(status);
		$("#trainWESelect").val(trainWeProductId);
		$("#visitnum").val(visitnum);
		$("#broadcastMode").val(broadcastMode);
		$("#trainWEProductParticipation").val(broadcastParticipation);
		$("#listViewContent").val(listViewContent_removeEnter);

		 $("#ImgPr").attr("src", "${pageContext.request.contextPath}/" + logo);
	}
	function TrainWEDelete(trainWEChildId) {
		if (confirm("确定要删除这条数据吗?")) {
			$.post("deleteTrainWEProductChild_AdminTrainWEChildAction.action", {
				trainWEChildId : trainWEChildId
			}, function(result) {
	...
	function deleteselectedSpans() {
		...
		if (confirm("您确定要删除这" + selectedSpan.length + "条数据吗？")) {
			$.post("deleteTrainWEChilds_AdminTrainWEChildAction.action", {
				ids : ids
			}, function(result) {
				if (result.success) {
					alert("数据已成功删除！");
...
<body>
	<div class="container-fluid">
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			...
			<form action="findByConditionChild_AdminTrainWEChildAction.action" method="post"
				class="form-search" style="display: inline;">
				&nbsp; 章节名称： <input name="title" value="" type="text"
					class="input-medium search-query" placeholder="输入名称...">
				&nbsp; 所属WE课堂： <select id="broadcastCProductHR"
					name="trainWeId"><option value="">请选择...</option>
					<c:forEach var="t" items="${trainWEProducts }">
				...
				<div class="widget-box">
					...
					<div class="widget-content nopadding">
						<table class="table table-bordered table-striped with-check">
							...
							<tbody>
								<c:forEach items="${trainWEProductChilds }" var="train" varStatus="idx">
									<tr>
										...
										<td style="text-align: center;vertical-align: middle;">${train.trainWEProduct.title }</td>
										<td
										...
										<td style="text-align: center;vertical-align: middle;">
											<button class="btn btn-info" type="button"
												...
												%> 
												onclick="return modifyTrainWE('${train.id }','${train.title }','${train.publisher}','${train.startTime }','${train.endTime }','${fn:replace(train.introduction, vEnter, nEnter) }','${fn:replace(train.listViewContent, vEnter, nEnter) }','${train.status }','${train.trainWEProduct.id }','${train.mode }','${train.participation==null?0:train.participation }','${train.imgUrl }','${train.visitnum }')">修改
											</button>&nbsp;&nbsp;
				...
				<form id="fm"
					...
					<table>
						...
						<tr>
							...
							<td><label class="control-label" for="trainWEName">章节名称：</label>
							</td>
							<td><input id="trainWEName" type="text"
								name="trainWEProductChild.title" placeholder="请输入…"></td>
						</tr>
						<tr>
							...
							<td><input id="trainWEPublisher" type="text"
								name="trainWEProductChild.publisher" placeholder="请输入…"></td>
						</tr>
						
						<tr>
							<td><label class="control-label" for="trainWEStartTime">开播时间：</label>
							</td>
							<td><input class="form_endtime" id="startinfo" type="text"
								placeholder="请选择" name="trainWEProductChild.startTime"></td>
						</tr>
						<tr>
							<td><label class="control-label" for="trainWEEndTime">结束时间：</label>
							</td>
							<td><input class="form_endtime" id="dateinfo" type="text"
								placeholder="请选择" name="trainWEProductChild.endTime"></td>
						</tr>
						...
						<tr>
							...
							<td>
							<select id="state" name="trainWEProductChild.status">
						...
						<tr>
							<td>
								<label class="control-label" for="trainWESelect">所属课程：</label>
							</td>
							<td>										
								  <select id="trainWESelect" name="trainWEProductChild.trainWEProduct.id">
								  	<option value=""></option>
									<c:forEach var="t" items="${trainWEProducts }">
										<option value="${t.id}" >${t.title }</option>
									</c:forEach>	
						...
						<tr>
							...
							<td><select id="broadcastMode" name="trainWEProductChild.mode">
						...
						<tr>
							<td><label class="control-label" for="sectionName">章节简介：</label>
							</td>
							<td>
							<textarea id="trainWEIntroduction" rows="5" cols="50" style="width: 405px;"  name="trainWEProductChild.introduction"></textarea>
							</td>
						</tr>
						<tr>
							<td><label class="control-label" for="sectionName">章节列表内容：</label>
							</td>
							<td>
							<textarea id="listViewContent" rows="5" cols="50" style="width: 405px;"  name="trainWEProductChild.listViewContent"></textarea>
							</td>
						</tr>						
						<tr>
							...
							<td><input id="visitnum" type="text"
								name="trainWEProductChild.visitnum"></td>
						</tr>


					</table>
					<input id="trainWEProductId" type="hidden" name="trainWEProductChild.id">
					<input id="trainWEProductParticipation" type="hidden" name="trainWEProductChild.participation" value="0">
				</form>
			</div>
