...
function openAddDlg(){
 ...
 function chooiseAdv(id){
		...
		$.post("adminCAdv_updateAdv.action",{id:id},
			function(result){
...
function zoneDelete(id){
	if(confirm("确定要删除这条数据吗?")){
		$.post("adminCAdv_deleteAdv.action",{id:id},
				function(result){
...
function deleteSections(){
	...
	if(confirm("您确定要删除这"+selectedSpan.length+"条数据吗？")){
		$.post("adminCAdv_deleteAdvs.action",{ids:ids},function(result){
			if(result.success){
				alert("数据已成功删除！");
				location.reload(true);
			}else{
...
<body>
	<div class="container-fluid">
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			...
			<form action="adminCAdv_findByStatus.action" method="post" class="form-search" style="display: inline;">      
			   &nbsp;广告状态：
		...
		<div id="dlg" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-body">
				<form id="fm" action="adminCAdv_addAdv.action"  method="post" enctype="multipart/form-data">
					<table>
						...
						<tr>
							<td>
								<label class="control-label" for="broadcastList">选择直播间：</label>
							</td>
							<td>
								 <select id="broadcastList" name="broadcastId">
									<option value="" >请选择</option>
									<c:forEach var="b" items="${broadcastList}">
