...
public class AdminTrainWEAdvAction extends BaseAction<TrainWEAdv> {
	private File adv;
	...
	public String showAdv() {
		...
		request.setAttribute("mainPage", "addTrainWEAdv.jsp");

		if (StringUtil.isEmpty(page)) {
			page = "1";
		}
		PageBean pageBean = new PageBean(Integer.parseInt(page), 6);
		List<TrainWEAdv> list = trainWEAdvService.findAdv("from TrainWEAdv",
				null, pageBean);
		Long count = trainWEAdvService
				.countAdvs("select count(*) from TrainWEAdv");
		String pageCode = PageUtil.genPagination(request.getContextPath()
				+ "/admin/showAdv_AdminTrainWEAdvAction.action", count,
				Integer.parseInt(page), 6, null);
		List<TrainWEProduct> blist = trainWEService.getAllTrainWEProducts();

		this.getSession().setAttribute("pageCode", pageCode);
		this.getSession().setAttribute("trainWEProduct", blist);
	...
	public String addAdv() {
		...
		System.out.println(trainWEId);
		if (StringUtil.isNotEmpty(trainWEId)) {
			TrainWEProduct b = trainWEService.getTrainWeById(trainWEId);
			trainWEService.evitTrainWEProduct(b);
		...
		trainWEAdvService.addAdvertisement(model);
	...
	public String deleteAdv() {
		...
		TrainWEAdv adv = trainWEAdvService.findAdvertisementById(model.getId());
		String path = request.getSession().getServletContext()
				.getRealPath("/" + adv.getImageUrl());
		File f = new File(path);
		f.delete();

		trainWEAdvService.deleteAdvertisement(adv);
	...
	public String deleteAdvs() {
		...
		for (int i = 0; i < str.length; i++) {
			...
			TrainWEAdv adv = trainWEAdvService.findAdvertisementById(Integer
					.parseInt(str[i]));
			String path = request.getSession().getServletContext()
					.getRealPath("/" + adv.getImageUrl());
			File f = new File(path);
	...
	public String findByStatus() {
		...
		PageBean pageBean = new PageBean(Integer.parseInt(page), 6);
		List<TrainWEAdv> list = trainWEAdvService.findPageByMap(
				"from TrainWEAdv", map, pageBean);
		Long count = trainWEAdvService
				.countAdvs("select count(*) from TrainWEAdv where status = "
						+ model.getStatus());
		String pageCode = PageUtil.genPagination(request.getContextPath()
				+ "/admin/findByStatus_AdminTrainWEAdvAction.action", count,
				Integer.parseInt(page), 6, null);
	...
	public String updateAdv() {
		...
		if (adv.getStatus() == 0) {
			...
			adv.setStatus(1);
			trainWEAdvService.saveAdvs(adv);
			return "updateAdv";
		}
		if (adv.getStatus() == 1) {
			adv.setStatus(0);
