...
function openAddDlg(){
 ...
 function modifyZone(id,name,telephone,email,job,background,introduction){
 	
 	var introduction_removeEnter = introduction.replace(/\[enter\]/g,"\n");
	 $("#myModalLabel").html("修改讲师");
	 $("#id").val(id);
	 $("#name").val(name);
	 $("#email").val(email);
     $("#job").val(job);
...
function zoneDelete(id){
	if(confirm("确定要删除这条数据吗?")){
		$.post("adminTeacher_deleteBTeacher.action",{id:id},
				function(result){
...
function deleteSections(){
	...
	if(confirm("您确定要删除这"+selectedSpan.length+"条数据吗？")){
		$.post("adminTeacher_deleteTeachers.action",{ids:ids},function(result){
			if(result.success){
				alert("数据已成功删除！");
...
<body>
	<div class="container-fluid">
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			...
			<form action="adminTeacher_findByMap.action" method="post" class="form-search" style="display: inline;">
	          &nbsp;讲师名：
			  <input name="name" value="${s_section.name }" type="text" class="input-medium search-query" placeholder="输入讲师名称..."/>
			  &nbsp;讲师电话：
			  <input name="telephone" value="${s_section.name }" type="text" class="input-medium search-query" placeholder="输入讲师电话..."/>
			  &nbsp;讲师邮箱：
			 <input name="email" value="${s_section.name }" type="text" class="input-medium search-query" placeholder="输入讲师邮箱..."/>
			  &nbsp;
				...
				<div class="widget-box">
					...
					<div class="widget-content nopadding">
						<table class="table table-bordered table-striped with-check">
							...
							<tbody>
								<c:forEach items="${teachers }" var="t" varStatus="idx">
									<tr>
										...
										<td style="text-align: center;">
												...
												%> 
											<button class="btn btn-info" type="button" data-backdrop="static" data-toggle="modal" data-target="#dlg" onclick="return modifyZone('${t.id }','${t.name }','${t.telephone }','${t.email }','${t.job }','${t.background }','${fn:replace(t.introduction, vEnter, nEnter) }')">修改
		...
		<div id="dlg" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-body">
				<form id="fm" action="${pageContext.request.contextPath}/adminTeacher_addBTeacher.action"  method="post" enctype="multipart/form-data">
					<table>
						<tr>
						...
						<tr>
							...
							<td>
								 <input id="password" type="text" name="password" placeholder="请输入…">
							</td>
						</tr>
						<tr>
							<td>
						...
						<tr>
							...
							<td>
								  <input id="email" type="text" name="email" placeholder="请输入…">
							</td>
						</tr>
						<tr>
							<td>
							...
							<td>
								<textarea rows="5" cols="50" style="width: 405px;" id="introduction" name="introduction"></textarea>
							</td>
						</tr>
					</table>
