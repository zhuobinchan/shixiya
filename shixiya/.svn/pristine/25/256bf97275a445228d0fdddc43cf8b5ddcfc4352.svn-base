...
<!--去掉手机强制某些数值识别成链接-->
<title style="text-align:center">${ cpBroadcast.name}</title>
...
<body>
	...
	<c:if test="${cpBroadcast.mode == 0 || cpBroadcast.mode == 1 }">
		<div class="video">
			<video id="video" width="100%" height="100%" ${ cpBroadcast.status==1?'controls':''}
			poster="${cpBroadcast.status==1?'/shixiya/omg/images/video/ready.png':'/shixiya/omg/images/video/wait.png' }"
			webkit-playsinline> 
				
				<c:if test="${(!empty session.hr.name) || (!empty session.teacher.name) }">
					<source id="videoSource" src="${ cpBroadcast.videoUrl}" type="video/mp4" /> </video>
					<source id="videoSource" src="${ cpBroadcast.videoUrl}" type="video/ogg" /> </video>
				</c:if>
				<c:if test="${empty session.hr.name && empty session.teacher.name }">
					<source id="videoSource" src="${ cpBroadcast.status==1?cpBroadcast.videoUrl:''}" type="video/mp4" /> </video>
					<source id="videoSource" src="${ cpBroadcast.status==1?cpBroadcast.videoUrl:''}" type="video/ogg" /> </video>
				</c:if>	
...
<div class="chatroom">	
	...
	<c:if
		test="${(empty session.student) && (!empty session.hr.name) || (!empty session.teacher.name) }">
	<div class="send">
	    <div class="send1">
	        <div class="send-word">
	    ...
	    <div class="send2">
	        ...
	        <div><div class="${cpBroadcast.chatStatus==1?'quiet':'noise' }" id="stopTalkBtn" status="${cpBroadcast.chatStatus }" ></div></div>
	...
	<c:if test="${cpBroadcast.mode == 0 || cpBroadcast.mode == 2}">
		<c:if
			test="${(!empty session.student) && (empty session.hr.name) && (empty session.teacher.name) }">
			<div class="send" ${ cpBroadcast.chatStatus==1?"":"style='display:none'" }>
			    <div class="send1">
			        <div class="send-word">
			            <input type="text" class="input-word"/>
			            <!--<i class="ft"></i>-->
